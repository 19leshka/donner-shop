...
const Cart = (props) => {
    ...
    const cartItems = [];
    const price = totalPrice === 0 ? 0 : totalPrice.toFixed(1)

    const itemMinus = (id, key) => {
        dispatch(setTotalPrice(totalPrice - items[key][0].price))
        dispatch(decrementDoner({id, key}));
    }
    const itemPlus = (id, key) => {
        dispatch(setTotalPrice(totalPrice + items[key][0].price))
        dispatch(incrementDoner({id, key}));
    }
    ...
    const deleteItemsGroupHandler = (id, key) => {
        dispatch(setTotalPrice(totalPrice - items[key][0].price * items[key].length))
        dispatch(setTotalCount(totalCount - items[key].length))
        dispatch(deleteItemsGroup({id, key}))
    }

    for(let key in items){
        let item = items[key][0];
            cartItems.push(<div key={key} className="cart__item">
                    ...
                    <div className="cart__item-info">
                        <h3>{item.name}</h3>
                        <p>{item.type}, {item.size} г.</p>
                    </div>
                    <div className="cart__item-count">
                        <div onClick={() => itemMinus(item.id, key)} className="button button--outline button--circle cart__item-count-minus">
                        ...
                        <b>{items[key].length}</b>
                        <div onClick={() => itemPlus(item.id, key)} className="button button--outline button--circle cart__item-count-plus">
                            <svg width="10" height="10" viewBox="0 0 10 10" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M5.92001 3.84V5.76V8.64C5.92001 9.17016 5.49017 9.6 4.96001 9.6C4.42985 9.6 4.00001 9.17016 4.00001 8.64L4 5.76L4.00001 3.84V0.96C4.00001 0.42984 4.42985 0 4.96001 0C5.49017 0 5.92001 0.42984 5.92001 0.96V3.84Z"
                    ...
                    <div className="cart__item-price">
                        <b>{(item.price * items[key].length).toFixed(1)} $</b>
                    </div>
                    <div className="cart__item-remove">
                        <div onClick={() => deleteItemsGroupHandler(item.id, key)} className="button button--outline button--circle">
                            <svg width="10" height="10" viewBox="0 0 10 10" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M5.92001 3.84V5.76V8.64C5.92001 9.17016 5.49017 9.6 4.96001 9.6C4.42985 9.6 4.00001 9.17016 4.00001 8.64L4 5.76L4.00001 3.84V0.96C4.00001 0.42984 4.42985 0 4.96001 0C5.49017 0 5.92001 0.42984 5.92001 0.96V3.84Z"
            ...
            <div className="cart">
                ...
                <div className="cart__bottom">
                    ...
                    <div className="cart__bottom-buttons">
                        <NavLink to="/donner-shop/" className="button button--outline button--add go-back-btn">
